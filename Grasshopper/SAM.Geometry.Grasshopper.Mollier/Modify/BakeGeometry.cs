...
namespace SAM.Geometry.Grasshopper.Mollier
{
    public static partial class Modify
    {
        public static void BakeGeometry(this RhinoDoc rhinoDoc, ChartType chartType, IEnumerable<IMollierProcess> mollierProcesses, IEnumerable<IMollierPoint> mollierPoints)
        {
            ...
            Layer layer_SAM = Core.Rhino.Modify.AddLayer(layerTable, string.Format("SAM{0}_", chartType.Description()));
            if (layer_SAM == null)
            {
                return;
            }
            ...
            if(mollierProcesses != null && mollierProcesses.Count() != 0)
            {
                ...
                foreach (IMollierProcess mollierProcess in mollierProcesses)
                {
                    ...
                    Layer layer = Core.Rhino.Modify.GetLayer(layerTable, layer_Process.Id, name, SAM.Geometry.Mollier.Query.Color(mollierProcess));
            ...
            if(mollierPoints != null && mollierPoints.Count() != 0)
            {
                ...
                foreach (IMollierPoint mollierPoint in mollierPoints)
                {
                    ...
                    Point3d point3d = SAM.Core.Grasshopper.Mollier.Convert.ToRhino_Point3d(mollierPoint, chartType, 0);
