...
namespace SAM.Core.Mollier
{
    ...
    { 
        ...
        public static List<ConstantTemperatureCurve> ConstantTemperatureCurves_DryBulbTemperature(Range<double> dryBulbTemperatureRange, Range<double> humidityRatioRange, double step, double pressure)
        {
            if (dryBulbTemperatureRange == null || double.IsNaN(dryBulbTemperatureRange.Min) || double.IsNaN(dryBulbTemperatureRange.Max) || double.IsNaN(step) || double.IsNaN(pressure))
            {
                return null;
            ...
            while (dryBulbTemperature <= dryBulbTemperatureRange.Max)
            {
                ConstantTemperatureCurve constantTemperatureCurve = ConstantTemperatureCurve_DryBulbTemperature(dryBulbTemperature, humidityRatioRange, pressure);
                if (constantTemperatureCurve != null)
                {
                    result.Add(constantTemperatureCurve);
                }
